...
from pipeline.utils import ImageNetHelper
import numpy as np
import progressbar
import json
import cv2

# initialize the ImageNet helper and use it to construct
...
# initialize the list of Red, Green, and Blue channel avergae
(R, G, B) = ([], [], [])
...
for (dType, paths, labels, outputPath) in datasets:
    ...
    widgets = ["Building List:", progressbar.Percentage(), " ", progressbar.Bar(),
        " ", progressbar.ETA()]
    ...
    for (i, (path, label)) in enumerate(zip(paths, labels)):
        ...
        # each channel in the image, then update the repsective lists
        if dType == "train":
...
# construct a dictionary pf averages, then serialize the means to a JSON file
