...
# devikit are store on disk
BASE_PATH = "/media/sunny/Seagate Expansion Drive/ImageNet/ILSVRC2015"

# based on BASE path, derive the images base path,
# image sets path, and devkit path
...
# define the path that maps the 1000 possible
# WordNet IDs to the class label integers
WORD_IDS = path.sep.join([DEVKIT_PATH, "map_clsloc.txt"])

# define the paths to the training file that maps
# the image filename to integer class label
TRAIN_LIST = path.sep.join([IMAGE_SETS_PATH, "train_cls.txt"])

# define the paths to the validation filenames along with
...
# since we do not have access to the testing data, we need to take a
# number of images from the training dat and use it instead
NUM_CLASSES = 1000
NUM_TEST_IMAGES = 50 * NUM_CLASSES

# define the path to the output trainingm validation, and testing lists
MX_OUTPUT = "/media/sunny/Seagate Expansion Drive/ImageNet/ILSVRC2015"
