...
class ImageNetHelper:
    ...
    def buildClassLabels(self):
        # load the contents of the file that maps the WordNet WORD_IDs
    ...
    def buildBlacklist(self):
        ...
        # return the blacklist image IDs
        return rows

    def buildTrainingSet(self):
        # load the contents of the training input file that lists the image ID
        ...
        for row in rows:
            # break the row into the partial path and iimage number
            ...
            path = os.path.sep.join([self.config.IMAGES_PATH, "train",
    ...
    def buildValidationSet(self):
        ...
        # load the contents of the file that lists partial validation image filenames
        valFilenames = open(self.config.VAL_LIST).read()
        valFilenames = valFilenames.strip().split("\n")

        # load the contents of the file that contains the actual ground-truth
        ...
        for (row, label) in zip(valFilenames, valLabels):
            ...
            # if the image number is in the blacklist set then
            # we should ignore this validation image
            if imageNum in self.valBlacklist:
                continue

            # construct the full path to the validtaion image
            # then update respective paths and labels lists
